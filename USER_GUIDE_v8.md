...
> 歡迎來到 DROS 8.0.0 (Epistemic Edition) 法義推理與認識論作業系統。本系統將極度硬核的 **36,000+ 個（36,057 個）** 核心概念節點，化為您本機隨身、忠誠的雙軌制法義伴學。
> 
...
| :--- | :--- | :--- | :--- |
| 🧠 **`core/`** | **知識核心 (The Brain)** | 🚨 **嚴禁更動** | 存放 3.6 萬+ 個黃金概念節點。這是 AI 的大腦，包含洗滌後的 Properties 格式 concepts。 |
...
> 
> 相對地，完整的「大覺藏集」高達 **1.6GB 以上**。若您是佛學深究者，或是想完整查閱所有大覺藏集內容的使用者，歡迎您自官方站點下載完整版，並將其放入 `Vault_DajueZang/` 資料夾中進行**「自由掛載」**！  
> 🏺 **大覺藏集（完整版 1.6GB）官方高速下載站點**：  
> 👉 [https://dl.mahabodhi.co/d?id=bSgtD7YPWhEmF6Z](https://dl.mahabodhi.co/d?id=bSgtD7YPWhEmF6Z)
>
> ⚠️ **📱 手機端 (iOS / Android) 效能與記憶體保護提醒**：
> - **建議配置**：手機端極力推薦**僅載入 Core Vault（預設核心庫）**，內建 3.6 萬節點隨身攜帶極速秒開，即使 3GB RAM 的 iPhone SE 2 亦能如絲順滑。
> - **大覺藏在手機之載入注意**：若將 1.6GB 完整大覺藏放進手機，因古漢語文字量極為龐大，若手機實體 RAM 較小（RAM ≤ 4GB），初次索引時可能觸發系統之記憶體防禦機制（如 iOS Jetsam）而在 75% 處閃退重啟。
> - **手機端掛載秘訣**：若手機需要存放完整大覺藏，請進入 Obsidian「設定 ➔ 檔案與連結 ➔ 排除資料夾 (Excluded files)」，將 `Vault_DajueZang` 加入排除。如此手機端將僅索引 3.6 萬核心節點，兼具極速響應與經文目錄翻閱！

### 🔍 Graphify 實際掃描範圍與動態引用

`DROS Doctrinal Copilot` 的 Graphify 檢索不是「存入特定資料夾就建立離線圖庫」，也沒有 `Insights/` 專屬觸發器。外掛在**每次提問**時呼叫 Obsidian 的 Markdown 檔案清單，依檔名候選進行即時匹配；符合的筆記才會被讀取並注入當次提示詞。已存入 Vault 的新筆記，下一次提問即可參與匹配，不需要另外執行索引或重建圖庫。

目前程式碼中的路徑白名單如下（路徑比對不分大小寫；根目錄底下的子資料夾也會納入）：

| 根路徑 | Graphify 行為 |
| :--- | :--- |
| `core/` | ✅ 掃描核心概念與長經典 Markdown 節點 |
| `user_pavilion/`（實體資料夾為 `User_Pavilion/`） | ✅ 掃描個人筆記，包含 `Insights/` 等所有子資料夾 |
| `vault_dajuezang/`（實體資料夾為 `Vault_DajueZang/`） | ✅ 掃描大覺藏 Markdown 節點 |
| `00_黃金索引庫/` | ✅ 掃描黃金索引 Markdown 節點（若 Vault 內存在） |
| `ai 總論/` | ✅ 掃描該路徑 Markdown 節點（若 Vault 內存在） |
| `ai 龍樹/` | ✅ 掃描該路徑 Markdown 節點（若 Vault 內存在） |
| 其他路徑 | ❌ 不列入 Graphify 候選 |

這個「動態累積」應理解為**查詢時即時重掃候選檔案並按需讀取**，不是背景監看、資料夾觸發或預先持久化的圖資料庫。Graphify 仍會依當次查詢產生的核心／相關節點候選，使用檔名精確、包含及漢字重疊分數（最低門檻 `0.4`）挑選檔案；候選命中後才讀取內容，並受單節點與總上下文長度限制。個人筆記可被引用，但仍屬個人層資料，不會因此升格為大覺藏或其他權威原典。
...
> - **白名單先行**：Graphify 會先以 `core/`、`User_Pavilion/`、`Vault_DajueZang/`、`00_黃金索引庫/`、`ai 總論/`、`ai 龍樹/` 六個根路徑（含子資料夾）篩選候選，再以檔案名稱進行名相匹配。
> - **路徑不是完全無關**：放入上述白名單以外的資料夾，不會成為 Graphify 候選；放在白名單內的錯誤子資料夾則仍可能被檔名匹配。此機制只決定檢索可見性，不會改變筆記的 authority / personal 資料層級。
