...
> 💡 **Important Device Notice (Mobile users should NOT download `.exe` files)**:  
> `.exe` is a binary installer format exclusive to Microsoft Windows PCs; **mobile operating systems (Apple iOS/iPadOS and Android) cannot execute `.exe` files**.  
> Mobile users should directly install the official **Obsidian App** from the official App Store / Google Play, then import this repository folder. Mobile devices run on the exact same **native TypeScript Zero-Ops architecture—100% Python-free** and ready out of the box!
