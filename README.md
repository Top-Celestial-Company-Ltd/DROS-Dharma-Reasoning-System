...
> 內建 28 部大乘核心原典 ＋ 36,000+ 佛學名相節點 ＋ **雙外掛預先配置**（DROS Doctrinal Copilot v1.1.1 伴讀外掛 ＋ Force Read Mode 義理原典防誤觸唯讀外掛，預設自動啟用）：  
...
>  
> 相對地，完整的「大覺藏集」高達 **1.6GB 以上（7,600+ 部典籍）**。若您是佛學深究者，或是想完整查閱所有大覺藏集內容的使用者，歡迎您自行下載完整版，並將其放入根目錄預留之 `Vault_DajueZang/` 資料夾中進行**「自由掛載」**（內附標準存放拓撲說明，TS 外掛會在查詢時自動以毫秒級速度原生穿透）！
>
> ⚠️ **📱 手機端重要硬體建議與防退機制提醒**：  
> - **手機端強烈推薦使用預設 Core Vault**：隨附的 3.6 萬核心概念節點在任何 iPhone（包含 3GB RAM 之 iPhone SE 2 / 舊款機型）與 Android 手機上均可秒開、順暢對話。  
> - **1.6GB 完整大覺藏建議於電腦端 (PC / Mac) 掛載**：因大覺藏高達 1.6GB 巨量古漢語純文字，手機端 Obsidian 於初次全文建立索引時需消耗逾 1.5GB 運算記憶體。若設備為記憶體較小之舊型手機（RAM ≤ 4GB），系統容易觸發 iOS Jetsam 記憶體保護機制而於 75% 處閃退中斷。若需在手機端閱讀大覺藏，建議於 Obsidian「設定 ➔ 檔案與連結 ➔ 排除資料夾」中將 `Vault_DajueZang` 設為排除索引，即可兼顧順暢檢索與經文翻閱。
